...
function onWindowResize() {
    ...
    // Adjust cube size based on window width (for example)
    const scaleFactor = Math.min(window.innerWidth / 1000, 0.7);  // Scale factor based on window size
    cube.scale.set(scaleFactor, scaleFactor, scaleFactor);  // Uniform scaling
}

// Add the resize event listener
window.addEventListener('resize', onWindowResize);

// Call initially to set the correct size on page load
onWindowResize();
